...
function RequiredUsers(Models) {
  ...
  function *ensureAnonymousUser() {
    ...
    try {
      Models.anonymousUser = yield User.get(global.systemContext, {emails:'anonymous@anonymous.int'});
    } catch (e) {
  ...
  function *ensureAdministratorUser() {
    ...
    try {
      let administrator = yield User.get(global.systemContext, {emails:['admin@admin.int']});
    } catch (e) {
