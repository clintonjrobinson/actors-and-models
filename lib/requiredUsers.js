...
function RequiredUsers(Models) {
  function *ensureSystemUser() {
    ...
    Models.systemUser = yield Models.mongo.findOne(User.collectionName, {emails:'system@system.int'});

    //No System user found.  Create one.
    if (!Models.systemUser) {
      let user = {
        emails: ['system@system.int'],
        name: 'System',
  ...
  function *ensureAnonymousUser() {
    ...
    try {
      Models.anonymousUser = yield User.get(Models.systemContext, {emails:'anonymous@anonymous.int'});
    } catch (e) {
      console.error(e);
      ...
      Models.anonymousUser = yield User.create(
        Models.systemContext,
        new User({
          emails: ['anonymous@anonymous.int'],
          name: 'Anonymous',
          password: utils.guid(32),
          roles: ['Anonymous']
        }),
  ...
  function *ensureAdministratorUser() {
    ...
    try {
      let administrator = yield User.get(Models.systemContext, {emails:['admin@admin.int']});
    } catch (e) {
