...
class Document extends Common {
  ...
  static *get(context, query, opts) {
    ...
    var ret = this.isClient
      ? yield command(`${this.API_PREFIX}${this.name}/${query._id}/get`)
  ...
  static *remove(context, doc) {
    ...
    var ret = this.isClient
      ? yield command(`${this.API_PREFIX}${this.name}/${id}/remove`)
  ...
  static *update(context, doc, opts) {
    ...
    var ret = this.isClient
      ? yield command(`${this.API_PREFIX}${this.name}/${doc._id}/update`, this.delta(doc))
      : yield mongo.findAndModify(this.collectionName, {_id: doc._id}, {$set: this.delta(doc)}, {new: true, fields: opts.fields})
    ;
  ...
  static *create(context, doc, opts) {
    ...
    var ret = this.isClient
      ? yield command(`${this.API_PREFIX}${this.name}/create`, doc.toJSON())
  ...
  static *count(context, query) {
    ...
    return this.isClient
      ? yield command(`${this.API_PREFIX}${this.name}/count`, query)
   ...
   * @returns {Promise}
   */
  static *find(context, query, fields) {
    var canAccess = this.canAccess(context, 'read');
    ...
    fields = fields || {};
    fields = this.secureRead(context, fields);

    var cursor = this.isClient
      ? yield command(`${this.API_PREFIX}${this.name}/count`, query)
      : yield mongo.find(this.collectionName, query, fields)
    ;
