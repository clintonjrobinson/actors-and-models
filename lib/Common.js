...
class Common {
  ...
  __getter(name) {
    ...
    if (property.type.name === "LocaleString") {
      if (this.__data[name]) {
  ...
  __setter(name, value) {
    ...
    if (property.type.name === 'LocaleString' && this.__data[name]) {
  ...
  toJSON() {
    ...
    var ret = {_type: this.constructor.name};
  ...
  static _clientInit() {
    ...
    for (var property in this._def.properties) {
      this.def.properties[property] = new Property(this._def.properties[property], property);
    }
...
Common.registerDefinition = function(def, isModel) {
  ...
  for (var name in def.properties) {
    ...
    if (def.properties[name].type === 'self' || def.properties[name].type == cls.name) {
      def.properties[name].type = cls;
    }
