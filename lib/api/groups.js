  ...
  Models.api.group = function *(userId, command, group, groupRole, toggleRole) {
    ...
    if (command === 'remove' && !groupRole) {
      ...
      if (command === 'remove' && userGroup.roles.length === 1 && userGroup.roles[0] === groupRole) {
        ...
        update =  {$set: {'groups.$.roles': userGroup.roles, '$groups.$.updated': new Date()}};
        ...
        //Otherwise we are adding or removing just a role.
        update = (command === 'add')
          ? {$addToSet: {'groups.$.roles': groupRole}, $set: {'groups.$.updated': new Date()}}
          : {$pull: {'groups.$.roles': groupRole}, $set: {'groups.$.updated': new Date()}}
        ;
      }
    } else if (command === 'add') {
      //Other wise, we are creating a whole new group record for them
      update = {$push: {'groups': {group: group, roles: [groupRole], created: new Date(), updated: new Date}}};
    } else {
