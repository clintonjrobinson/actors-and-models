...
class Document extends Common {
  ...
  static *create(context, doc, opts) {
    ...
    if (this.hasGroupSecurity && this.def.secure && this.def.secure.create && !utils.userHasRole(context.user, this.def.secure.create)) {
      groups = [];

      for (let group of doc._groups) {
        if (utils.userInGroup(group)) {
          groups.push(group);
        }
      }
    }
