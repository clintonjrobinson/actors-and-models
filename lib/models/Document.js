...
class Document extends Common {
  ...
  static *get(context, query, opts) {
    ...
    if (this.isClient) {
      ret = yield command(`${this.API_PREFIX}${this.cls}/${query._id}/get`)
    } else {
      ret = yield this.mongo.findOne(this.collectionName, query, opts);
    }
  ...
  static *remove(context, doc) {
    ...
    yield this.middleware.beforeRemove.call(query, context);

    let ret;

    if (this.isClient) {
      let response = yield command(`${this.API_PREFIX}${this.cls}/${id}/remove`);
      ret = new this(response);
    } else {
      let response = yield this.mongo.findOneAndDelete(this.collectionName, {_id: id});
      ret = new this(response.value);
    }

    yield this.middleware.afterRemove.call(ret, context);
  ...
  static *update(context, id, updateSet, opts) {
    ...
    if (this.isClient) {
      ...
      let fromServer = yield command(`${this.API_PREFIX}${this.cls}/${id._id}/update`, this.delta(id));
    ...
    let data = yield this.mongo.findOne(this.collectionName, {_id: id});
    ...
    let query = this.queryFromUpdateSet(this.delta(doc));
    console.log(JSON.stringify(query));

    let result = yield this.mongo.findOneAndUpdate(this.collectionName, {_id: doc._id}, query, opts);
    let ret = new this(result.value);

    yield this.middleware.afterSave.call(ret, context, doc);
    yield this.middleware.afterUpdate.call(ret, context, doc);
  ...
  static *create(context, doc, opts) {
    ...
    var data = this.isClient
      ...
      : yield this.mongo.insert(this.collectionName, doc.toJSON())
    ;
  ...
  static *count(context, query) {
    ...
    return this.isClient
      ...
      : yield this.mongo.count(self.collectionName, query)
    ;
  }
  ...
  static *find(context, query, options) {
    ...
    if (this.isClient) {
      cursor = yield command(`${this.API_PREFIX}${this.cls}/find`, query);
    } else {
      cursor = yield this.mongo.find(this.collectionName, query, options);
    }
