...
class Document extends Common {
  ...
  static *get(context, query, opts) {
    ...
    if (canAccess === undefined && this.hasOwnerSecurity) {
      ...
      query._groups = {$in: context.userGroups};
    }
  ...
  static *update(context, doc, opts) {
    ...
    opts = opts || {};
    opts.fields = this.secureRead(context, opts.fields, securityDoc);
    opts.upsert = false;
    opts.multi = false;
    opts.new = true;
    ...
    this.secureUpdate(context, doc, securityDoc);
    ...
    var ret = this.isClient
      ...
      : yield mongo.findOneAndUpdate(this.collectionName, {_id: doc._id}, {$set: this.delta(doc)}, opts)
    ;
  ...
  static *create(context, doc, opts) {
    //Owner security and group security don't apply to a new doc, they can't.
  ...
  static get def() {
    if (!this.__def) {
      ...
      if (this.hasGroupSecurity) {
        this.__def._groups = new Property({
          type: ObjectID,
          array: true,
          secure: {
            update: ['System']
      ...
      if (this.hasOwnerSecurity) {
        this.__def._owner = new Property({
          type: ObjectID,
          secure: {
            update: ['System', 'Owner']
          }
