...
function cast(ctx, obj, value, params) {
  ...
  if (obj === String || obj === Number || obj === Date || obj === Boolean) {
    if (value === undefined) {
      return undefined;
    }
...
class Projection {
  ...
  get paginate() {
    return this.def.paginate || false;
  }
  ...
  async secure(ctx, params) {
    if (this.def.secure) {
      return await this.def.secure.call(this, ctx, params);
    }

    return true;
  }

  async fetch(ctx, params, opts) {
    function inject(paramName, paramValue, parent) {
      for (let field in parent) {
        ...
        if (parent[field] === null) {
          ...
          parent[field].constructor === String &&
          parent[field].indexOf('${' + paramName + '}') !== -1
        ) {
    ...
    for (let name in this.def.fetch) {
      ...
      let query =
        fetch.query && fetch.query.constructor === Function
          ? fetch.query(params)
          : JSON.parse(JSON.stringify(fetch.query));

      let options = fetch.options
        ? JSON.parse(JSON.stringify(fetch.options))
        : {};
      ...
      if (this.def.beforeFetch) {
        await this.def.beforeFetch.call(
          this,
          ctx,
          fetch,
          params,
      ...
      //console.log(`${this.def.name}`, query, options);

      if (this.def.directAccess) {
        let cursor = await fetch.model.mongo.find(
          fetch.model.collectionName,
          query,
          options
        );
        ret[name] = await cursor.toArray();
      } else {
        ret[name] = await fetch.model.find(ctx, query, options);
      }

      if (this.paginate) {
        ret.count = await fetch.model.mongo.count(
          fetch.model.collectionName,
          query
        );
