      ...
      if (this.__data[property.name] !== undefined) {
        if (property.isArray && this.__data[property.name] !== null) {
          ...
          for (let i = 0; i < this.__data[property.name].length; i++) {
            ret[property.name][i] = this.__data[property.name][i] ? this.__data[property.name][i].toJSON() : null;
          }
        } else {
          ret[property.name] = this.__data[property.name] ? this.__data[property.name].toJSON() : null;
        }
      }
    }
            ...
            for (let i=0; i<arr.length; i++) {
              if (arr[i]._mg === mg) {
                if (arr[i].constructor.patch) {
                  arr[i].constructor.patch(context, arr[i], update.sub[mg], roles);
                } else {
        ...
        for (let i=0; i<update.pull.length; i++) {
          for (let j=0; j<arr.length; j++) {
            ...
            if (property.isSubDocument && arr[j]._mg === update.pull[i].val) {
              arr.splice(j, 1);
            } else if (property.isReference && arr[j]._id.equals(update.pull[i].val)) {
              arr.splice(j, 1);
            } else if (property.type === Types.ObjectID && arr[j].equals(update.pull[i].val)) {
              arr.splice(j, 1);
            } else if (property.isType) {
