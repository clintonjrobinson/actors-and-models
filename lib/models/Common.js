...
class Common {
        ...
        if (change.type === 'splice') {
          ...
          if (change.addedCount) {
            let obj = {
              ...
              cmd: 'addToSet',
              name: change.name,
              val: change.object.slice(change.index, change.index + change.addedCount)
            };
  ...
  static collapse(doc, name, sets, unsets, ignore) {
    ...
    for (let property of this.properties()) {
      ...
      if (val === null) {
        ...
        //noop
      } else if (property.isType || property.isReference) {
        sets[collapsedName] = val;
        sets.__count++;
      } else if (property.isSubDocument) {
        property.type.constructor.collapse(val, collapsedName, sets, unsets, true);
  ...
  static delta(doc) {
    ...
    for (let change of doc.__changes) {
      ...
      if (change.val === null) {
        upd[change.name] = null;
      } else if (property.isSubDocument) {
        //A new subdocument has been added.  The subdoc will contain all its own changes.
        if (change.cmd === 'add') {
          upd[change.name] = doc[change.name].toJSON();
        } else {
          upd[change.name] = change.val.constructor.delta(change.val);
        }
      } else if (property.isArray) {
