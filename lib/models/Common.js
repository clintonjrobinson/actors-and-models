...
class Common {
  ...
  _childChanges(property) {
    //Already noted!  Don't need to propagte
    if (!this.__changes[property.name]) {

      this.__changes[property.name] = {
        cmd: 'sub',
        val: property.isArray ? this.__data[property.name] : this.__data[property.name].__changes
      };
    }

    if (this.__parent) {
      this.__parent._childChanges(this.__property);
    }
  }
  ...
  static delta(doc) {
    for (let property of this.properties({isArray: true})) {
      ...
      if (doc.__arrays[property.name] && doc.__data[property.name]) {
        ...
        //Clean up
        if (changes.push.length === 0 && changes.pull.length === 0){
