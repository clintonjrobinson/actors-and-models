...
class Common {
  constructor(properties, locale, parent, property) {
    ...
    if (this.constructor.isStructure && (!properties || (properties && !properties._mg))) {
  ...
  toJSON() {
    ...
    for (let property of this.constructor.properties({isSubDocument: false, isType: false})) {
    ...
    for (let role of roles) {
      ...
      if (this.__secureRead[role]) {
        ...
        for (let property of this.properties()) {
          ...
          //down the chain
          if (secureRole[property.name] && property.isModel && property.hydrate) {
            let subFields = property.type.secureRead(context, fields ? fields[property.name] : null, doc);
  ...
  static justTheData(doc) {
    ...
    for (let property of this.properties({isType:true})) {
      if (doc.__data[property.name]) {
    ...
    for (let name in updateSet) {
      ...
      if (update.cmd === 'unset') {
        ...
        if (property.isArray) {
          ...
          for (var i=0; i<update.val.length; i++) {
            if (property.isSubDocument) {
              query.$set[fullName][i] = update.val[i].constructor.justTheData(update.val[i]);
          ...
          query.$set[fullName] =  update.val ? update.val.constructor.justTheData(update.val) : undefined;
        } else if (property.isType) {
        ...
        for (var i=0; i<update.pull.length; i++) {
          ...
          query.$pullAll[fullName + update.pull[i].path] = query.$pullAll[fullName + update.pull[i].path] || [];
          query.$pullAll[fullName + update.pull[i].path].push(update.pull[i].val);
        }

        for (var i=0; i<update.push.length; i++) {
          query.$addToSet = query.$addToSet || {};
          query.$addToSet[fullName] = query.$addToSet[fullName] || {$each:[]};
    ...
    for (let name in updateSet) {
      ...
      if (!this.isClient) {
        ...
        if (!property.canUpdate(roles)) {
          console.log('cant update', property, roles);
          continue;
        }
      }
        ...
        if (update.sub) {
          for (let mg in update.sub) {
            for (let i=0; i<arr.length; i++) {
              if (arr[i]._mg === mg) {
                property.type.patch(context, arr[i], update.sub[mg]);
              }
            }
          }
        }
        ...
        for (let i=0; i<update.pull.length; i++) {
          for (let j=0; j<arr.length; j++) {
            ...
            if (property.isSubDocument && arr[j]._mg === update.pull[i].val) {
              arr.splice(j, 1);
            } else if (property.isReference && arr[j]._id.toString() === update.pull[i].val) {
              arr.splice(j, 1);
            } else if (property.type === Types.ObjectID && arr[j].toString() === update.pull[i].val) {
              arr.splice(j, 1);
            } else if (property.isType) {
              console.error('TODO: patch for array of Types not yet implemented');
  ...
  static delta(doc) {
    for (let property of this.properties({isArray: true})) {
      ...
      if (doc.__arrays[property.name] && doc.__data[property.name]) {
        ...
        for (let i = 0; i < doc.__data[property.name].length; i++) {
          ...
          for (let j = 0; j < doc.__arrays[property.name].length; j++) {
            ...
            if (property.isSubDocument && ival._mg === jval._mg) {
              ...
              console.error('TODO: delta on array of types not yet implemented');
            } else if (ival === jval) {
              found = true;
            }
        ...
        for (let j = 0; j < doc.__arrays[property.name].length; j++) {
          ...
          if (!found) {
            if (property.isSubDocument) {
              changes.pull.push({path: '._mg', val: jval});
            } else if (property.isReference) {
              changes.pull.push({path: '._id', val: jval});
            } else if (property.isType) {
              console.error('TODO: delta on array of types not yet implemented');
            } else {
        ...
        //Clean up
        if (changes.push.length === 0 && changes.pull.length === 0 && !changes.sub){
    ...
    for (let property of this.properties({isSubDocument: true, isArray: false})) {
      if (doc.__data[property.name]) {
        property.type.delta(doc.__data[property.name]);
      }
    }
  ...
  static _clientInit() {
    ...
    for (let hook of ['before', 'after']) {
      for (let method of ['Save', 'Create', 'Validate', 'Count', 'Find', 'Update', 'Remove']) {
...
Common.registerDefinition = function(def, isModel) {
  ...
  process.nextTick(function() {
    ...
    if (cls.hasGroupSecurity) {
      ...
      cls.def.indexes.push({key: {'_groups':1}, name:'groupsecurity', unique:false, sparse:false});
    }

    //Owner security requires an index
    if (cls.hasOwnerSecurity) {
      cls.def.indexes = cls.def.indexes || [];
      cls.def.indexes.push({key: {'_owner':1}, name:'ownersecurity', unique:false, sparse:false});
    }
