...
class Common {
  constructor(properties, locale, parent, property) {
    ...
    if (this.isStructure && (!properties || (properties && !properties._mg))) {
  ...
  _childChanges(property) {
    ...
    console.log(this.constructor.name, property.name);
    if (!property.isArray) {
      this.__changes[property.name] = {cmd: 'sub', val: this.__data[property.name].__changes};
    }
    ...
    for (let name in updateSet) {
      ...
      if (update.cmd === 'unset') {
        ...
        if (property.isSubDocument) {
          ...
          console.log(update.val, update.val.toJSON());
