...
class Common {
  ...
  _observe() {
    ...
    Object.observe(self.__data, function(changes) {
      ...
      for (let change of changes) {
        ...
        self.__changes.push(obj);
      }
    });
  }

  _observeArray(toObserve, toUpdate) {
    if (!toObserve) return;

    Array.observe(toObserve, function(changes) {
      console.error('TODO: Array.observe not implemented',changes);
    });
  }
  ...
  __setter(name, value) {
    ...
    Object.getNotifier(this).notify({
      type: (this.__data[name] === null || this.__data[name] === undefined) ? 'update' : 'add',
      name: name,
      object: this,
    ...
    //This is a sub doc, we need to know when something has changed to this doc and propogate it up
    //to the parent doc!
    ...
    if (property.array) {
      this._observeArray(this.__data[name], this.__data[name]);
    }
  }
  ...
  static *validate(doc, method) {
    ...
    if (this.isStructure || method === 'create' || (method === 'update' && this.canPatchUpdate) || (method === 'patch' && this.canPatchUpdate)) {
      ...
      let current = yield new this(this.mongo.findOne(this.collectionName, {_id:doc._id}));
   ...
   */
  static delta(doc) {
    //TODO: refactor delta to allow for other DB implementations.
    //There was no doc passed in, or the were no changes to the doc anyways.
    if (!doc || !doc.__changes) return;

    var upd = {};
    //Changes will already be ordered by time.
    for (let change of doc.__changes) {
      //Grab the property definition for this change.
      let property = this.def.properties[change.name];
      if (property.isSubDocument) {
        let delta = change.val.constructor.delta(change.val);
        //A new subdocument has been added.  The subdoc will contain all its own changes.
        if (change.type === 'add') {
          upd[change.name] = delta;
        } else {
          utils.collapseObject(delta, change.name, upd);
        }
      } else if (property.isArray) {
        console.error('TODO: delta for array not implented yet', change.name);
      } else if (property.isType) {
        upd[change.name] = change.val.toJSON();
      } else {
        upd[change.name] = change.val;
      }
    }
  ...
  static patch(doc, patch) {
    ...
    for (let property of this.properties({isSubDocument: true})) {
      if (patch[property.name] !== undefined) {
        ...
        if (property.isArray) {
          console.error('//TODO: patch array of subdocuments is not yet implemented');
          doc[property.name] = patch[property.name];
        } else {
          if (!doc[property.name]) {
            //There is no object existing in the doc. First create a new instance
            doc[property.name] = new property.type({_mg: patch[property.name]._mg});
          }
