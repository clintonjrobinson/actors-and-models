...
class Property {
   ...
   */
  cast (value, obj) {
    function subcast(value, obj, inst) {
      //This type is undefined, or its null, or it already is the destination format
      if (value === undefined || value === null || (value.constructor === this.type && !this.isReference)) {
        return value;
      }

      //This is a simple type that doesn't require casting
      ...
      if (this.type === Date) {
        ...
        if (value.add && value.add.constructor === Function) {
          return value;
        }
      ...
      if (this.isSubDocument) {
        ...
        if (value && value.__data && value.__data._type) {
          if (value.__data._type === value.constructor.cls) {
            ...
            value.__parent = obj;
            value.__property = this;
          }
          return value;
        }

        if (value && value._type && value._type !== this._type.cls) {
          //It has not been instantinated, but Model info was provided.
          return new this._type.children[value._type](value, obj.__locale, obj, this);
        }

        //All subdocs inherit the same locale.
        return new this._type(value, obj.__locale, obj, this);
      }
      ...
      if (this.isType) {
        ...
        if (this.type.hasValue && inst && inst.constructor === this.type && value.constructor === String) {
          inst.value = value;
          return inst;
        }

        return new this._type(value, obj, this);
      }
