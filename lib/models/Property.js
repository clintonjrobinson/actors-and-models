...
class Property {
      ...
      for (var name in self.validators) {
        ...
        if (!validator) {
          ...
          if (typeof self.validators[name] === 'function') {
            ...
            throw new Error(
              `Property ${self.name} has invalid validator ${name}.`
            );
          }
        } else {
  ...
  cast(value, obj) {
    function subcast(val, obj, inst) {
      ...
      if (
        val === undefined ||
        val === null ||
        (val.constructor === this.type && !this.isReference)
      ) {
        return val;
      }
      ...
      if (this.isSubDocument) {
        ...
        if (val && val._type && val._type !== this._type.cls) {
          ...
          return new this._type.children[val._type](
            val,
            obj.__locale,
            obj,
            this
  ...
  static serialize(property) {
    ...
    if (property.type.isType) {
      ...
      typeName = `"Models.${
        property.type.isStructure ? 'structures' : 'models'
      }.${property.type.cls}"`;
      ...
      typeName = `"Models.${property.type.split('.')[0].toLowerCase()}s.${
