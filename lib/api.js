...
exports = module.exports = function(Models) {
  const router = new require('koa-router')({
    prefix: Models.constants.API_PREFIX
  });

  const projectionApi = require('./api/projection')(Models);
  ...
  router
    ...
    .post('projection/:name', function *() {
      return yield projectionApi.call(this)
    })

    .post(':model/count', function*() {
      let model = Models.models[this.params.model];

      if (model.api && model.api.count === false)
        throw new Models.errors.APISecuredError();

      let opts = JSON.parse(
        JSON.stringify(this.request.fields ? this.request.fields.options : {})
      );
      opts.json = true;
      ...
      if (
        ...
        model.api.middleware.beforeCount
      ) {
        yield model.api.middleware.beforeCount.call(
          this,
          this.request.fields.query,
          opts
        );
      }
      ...
      if (
        model.api &&
        model.api.middleware &&
        model.api.middleware.afterCount
      ) {
    ...
    .post(':model/find', function*() {
      ...
      if (model.api && model.api.find === false)
        throw new Models.errors.APISecuredError();

      let opts = JSON.parse(
        JSON.stringify(
          this.request.fields && this.request.fields.options
            ? this.request.fields.options
            : {}
        )
      );
      opts.json = true;
      ...
      if (
        model.api &&
        model.api.middleware &&
        model.api.middleware.beforeFind
      ) {
        yield model.api.middleware.beforeFind.call(
          this,
          this.request.fields.query,
          opts
        );
    ...
    .post(':model/create', function*() {
      ...
      if (model.api && model.api.create === false)
        throw new Models.errors.APISecuredError();

      if (
        model.api &&
        model.api.middleware &&
        model.api.middleware.beforeCreate
      ) {
      ...
      if (
        model.api &&
        model.api.middleware &&
        model.api.middleware.afterCreate
      ) {
    ...
    .get(':model/:id/get', function*() {
      ...
      if (model.api && model.api.get === false)
        throw new Models.errors.APISecuredError();
    ...
    .get(':model/:id/remove', function*() {
      ...
      if (model.api && model.api.remove === false)
        throw new Models.errors.APISecuredError();
      ...
      if (
        model.api &&
        model.api.middleware &&
        model.api.middleware.beforeRemove
      ) {
      ...
      if (
        model.api &&
        model.api.middleware &&
        model.api.middleware.afterRemove
      ) {
    ...
    .post(':model/:id/update', function*() {
      ...
      if (model.api && model.api.update === false)
        throw new Models.errors.APISecuredError();
      ...
      if (
        model.api &&
        model.api.middleware &&
        model.api.middleware.beforeUpdate
      ) {
      ...
      if (
        model.api &&
        model.api.middleware &&
        model.api.middleware.afterUpdate
      ) {
    ...
    if (path.indexOf(Models.constants.PROPERTY_API_IDENTIFIER) === 0) {
      ...
      if (type && type.api && type.api[action]) {
        ...
        yield type.api[action].call(this, {
          id: id,
          model: model,
          propertyName: path.replace(
            Models.constants.PROPERTY_API_IDENTIFIER,
            ''
          ),
          property: property
        });
