  ...
  *fetch (ctx, inParams) {
    ...
    function inject(paramName, paramValue, parent) {
      for (let field in parent) {
        ...
        if (field === '${' + paramName + '}') {
          query[paramValue] = parent[field];
          delete parent[field];
        }

        //If this field matches the param name, inject the param value here
        if (parent[field].constructor === String && parent[field] === '${' + paramName + '}') {
    ...
    for (let param in this.def.params) {
      ...
      params[param] = new this.def.params[param](inParams[param]);
    ...
    for (let name in this.def.fetch) {
      ...
      let query = Object.assign({}, fetch.query);
      let options = Object.assign({}, fetch.options);
