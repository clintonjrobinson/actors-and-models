...
function generatePath(ctx, opts) {
  console.log(`${ctx.app.Models.config.root}/images/${opts.model.cls}/${opts.id}_${opts.propertyName.replace('.', '_')}${IMG_SUFFIXES[opts.type]}`);
...
Image.api = {
  ...
  create: function *(opts) {
    ...
    if (!(this.request.body && this.request.body.files && this.request.body.files[opts.propertyName])) {
      throw new Error(`Invalid propertyName.  Expecting ${opts.propertyName}`);
    }

    var metadata = JSON.parse(this.request.body.fields.metadata);
    opts.type = metadata.type;

    var file = this.request.body.files[opts.propertyName];
